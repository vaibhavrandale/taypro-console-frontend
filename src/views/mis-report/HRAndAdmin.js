...
import HRAndAdminModal from "./HRAndAdminModal";
...
export default function HRAndAdmin({
  ...
  const metrics = [
    ...
    { key: "exits", label: "Employee Exits" },
    { key: "training_hours", label: "Training Hours" },
    { key: "salary_expense", label: "Salary Expense" },
    { key: "travel_expense", label: "Travel Expense" },
    { key: "hotel_expenses", label: "Hotel Expenses" },
    { key: "employee_expenses", label: "Employee Expenses" },
    { key: "pending_hirings", label: "Pending Hirings" },
    { key: "total_disputes", label: "Total Disputes" },
  ];
            ...
            {metrics.map((m) => (
              <CCol key={m.key} xs={12} sm={5} md={4}>
                <motion.div
                  ...
                  className="p-4 bg-gray-900/60 rounded-2xl border border-gray-800 hover:border-indigo-500/50 transition-all duration-300"
                >
