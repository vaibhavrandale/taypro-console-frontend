...
const ClientUsersManagement = () => {
  ...
  const handleAdd = async () => {
    try {
      ...
      // Construct the assigned_sites object correctly
      const selectedSiteObj = sites.find((site) => site._id === selectedSite);
      const assignedSiteEntry = selectedSiteObj
        ? {
            _id: selectedSiteObj._id,
            siteName: selectedSiteObj.siteName,
            site_id: selectedSiteObj.site_id,
            assignedBy: {
              userId: userInfo._id, // Assuming you have the logged-in user's ID
    ...
    <div className="">
      ...
      >
        ...
        <CModalBody>
          ...
          />{" "}
          <CFormLabel>Assign Site </CFormLabel>
          <CFormSelect
            name="assigned_sites"
            value={selectedSite}
