...
const SitewaiseLog = () => {
  ...
  //i want  data in output like this  1h 30m 20s
  const getDuration = (finishedAt, startAt) => {
    const duration =
      new Date(finishedAt).getTime() - new Date(startAt).getTime();
  ...
  // Estimated one-way clean: row length (m) at 12 m/min
  const estimatedCleaningTime = (rowLengthMeters) => {
    const meters = Number(rowLengthMeters);
    if (!Number.isFinite(meters) || meters <= 0) return "N/A";
    const totalSeconds = Math.round((meters / 12) * 60);
                    ...
                    >
                      <CTableHead color="secondary">
                        <CTableRow>
                          ...
                          <CTableHeaderCell style={{ minWidth: "190px" }}>
                            Duration (Minutes -From Finished At Uplink)
                          </CTableHeaderCell>
                          <CTableHeaderCell style={{ minWidth: "190px" }}>
                            Total Time (Minutes -From Metric Data)
                          </CTableHeaderCell>
                          <CTableHeaderCell style={{ minWidth: "150px" }}>
                            Battery Start (%)
                          </CTableHeaderCell>
                        ...
                        {cleaning_completed.length > 0 ? (
                              ...
                              <CTableRow key={index}>
                                ...
                                <CTableDataCell>
                                  ...
                                  <span className="ms-2 text-muted">
                                    ({estimatedCleaningTime(log.row_length)})
                                  </span>
                                </CTableDataCell>
                          ...
                          <CTableRow>
                            <CTableDataCell colSpan="14" className="text-start">
                              No Completedlogs found for the selected date.
                            </CTableDataCell>
                          </CTableRow>
                        )}
                      </CTableBody>
